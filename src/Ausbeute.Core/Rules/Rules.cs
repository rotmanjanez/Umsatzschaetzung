...
public static class RuleCheck
{
    ...
    {
        foreach (var (id, e) in rs.Ingredients) ValidateIngredient(Keyed(id, e));
    ...
    static void ValidateIngredient(Ingredient e)
    {
    ...
    static void ValidateYieldRule(RuleSet rs, YieldRule e)
    {
        ...
        if (e.Default && e.Meta.ValidTo is null)
        {
            foreach (var (id, o) in rs.YieldRules)
            {
                ...
                if (ingredient == "" && string.IsNullOrEmpty(o.IngredientId) && (o.Category ?? "") == category)
